...
위 예시에서 커스텀 객체 range는 반복 가능하고 제너레티어 *[Symbol.iterator]엔 값을 나열해주는 로직이 구현되어 있다. 
...
- 클라이언트는 `https://apit.github.com/repos/<repo>/commits`형태의 URL로 요청을 보낸다
...
async function* fetchCommits(repo) {
  ...
  while(url) {
    const reponse = await fetch(url, { // (1)
      headers: {'User-agent': 'Our Script'}, // Github는 모든 요청에 user-agent헤더를 강제한다.
    });
    
    const body = await reponse.json(); // (2) 응답은 JSON형태로 온다.(커밋이 담긴 배열)
