...
이 때, `star.jpg`를 응답하는 서버는 헤더의 용량이 `0.1M`, 바디의 용량이 `1.0M`, 총 용량이 1.1M인 데이터(`star.jpg`)만큼의 네트워크를 차지하여 데이터(`star.jpg`)를 웹 브라우저에 렌더링하고 표시한다. 
...
이전과는 다르게 `cache-control`헤더가 추가된 것을 볼 수 있다. 헤더의 값에는 `max-age`가 있는데 이는 **캐시가 유효한 시간(초)**을 의미한다. 최초로 요청할 때, 서버는 `star.jpg`를 내려준다 웹 브라우저는 응답에 `cache-control`헤더를 보고 캐시로 인식하여 브라우저 캐시(캐시 저장소)에 이 응답 결과를 저장한다.

* 두 번째 요청

에서는 캐시를 먼저 조회한다.(*브라우저 캐시에서*) 60초 안에 유효하다면 캐시에서 응답 결과를 바로 가져온다(네트워크를 타지 않는다) 

* 세 번째 요청

60초가 초과 됐을때, 클라이언트는 다시 요청을 해야한다. 이렇게 되면 서버는 캐시 헤더와 함께 응답하고 첫 번째 요청과 같이 1.1M의 네트워크를 사용하여 캐시 저장소에 기존값을 덮어씌워 응답 결과(캐시 유효시간)를 저장한다. 이제 다시 60초 동안 유효 시간이 생긴다.
...
* 단 **클라이언트의 캐시데이터와 서버의 캐시 데이터가 같다는 사실을 확인할 수 있는 방법이 필요**하다.
...
검증 헤더`Last-Modified`를 추가하여 이 데이터가 마지막 수정된 시간을 추가하여 응답을 내려줄 수 있다.
...
에서 조건부 요청인 `if-modified-since` 헤더를 추가해서 요청한다.

* 응답

에서는 데이터 최종 수정일과 요청의 `if-modified-since`를 서로 비교하여 동일하다면  데이터가 아직 수정되지 않았음을 검증할 수 있다. 수정되지 않은 데이터는 서버에서 다음과 같은 응답을 내린다.
...
서버에서는 304 Not Modified 즉, 수정되지 않았다는 상태코드와 함께 `cache-control`, `Last-Modified`헤더를 응답하는데 **이 때 중요한 건 이 응답에는 HTTP Body가 없다는 것이다.** 그러면 바디를 제외한 헤더 부분만 응답하기 때문에 네트워크 부하가 줄어든다. 클라이언트에서는 상태 코드를 확인하고 캐시가 바뀌지 않았음을 인지하여 각 헤더를 갱신하며 캐시를 다시 세팅하고 브라우저 캐시를 조회하여 캐시를 사용하게 된다.
...
* 크게 2가지가 있다 -> Last-Modified, ETag 



### 조건부 요청 헤더란

* 클라이언트에서 검증 헤더로 조건에 따른 분기를 서버에 요청할 때 사용.
...
* 날짜 기반의 로직을 사용한다.(정해진 로직)
* 데이터를 수정해서 날짜가 다르지만, 같은 데이터를 수정해서 최종 데이터 결과가 똑같은 경우
* 서버에서 별도의 캐시 로직을 관리하고 싶은경우
  * 예) 스페이스 || 주석처럼 크게 영향이 없는 변경에 캐시를 유지하고 싶은 경우
