...
*예외를 메서드의 throws에 명시하는 것은 예외를 처리하는 것이 아니다. 호출한 메서드에게 예외를 전달함여 예외처리를 떠맡기는 것이다. 이렇게 떠넘겨지는 예외는 호출스택을 따라 전달되다가 제일 마지막에 있는 main 메서드에서도 예외가 처리되지 않으면 main 메서드 마저 종료되어 프로그램 전체가 종료된다.*
